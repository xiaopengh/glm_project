...
# Key predictors identified from EDA
...
# Model 1: Intercept only (baseline for comparison)
model_null <- lm(y ~ 1, data = train_data)

# Model 2: Parsimonious - Top predictors from EDA
# Based on EDA: study_hrs and attend_pct showed strongest associations
model_parsimonious <- lm(y ~ study_hrs + attend_pct + sleep_qual,
                         data = train_data)

# Model 3: Main effects - Key predictors
model_main <- lm(y ~ study_hrs + attend_pct + sleep_hrs + sleep_qual +
                   school_type + parent_educ + extra_act,
                 data = train_data)

# Model 4: Full main effects
...
# Forward stepwise from null model
...
message("\nStepwise Model Adj R²: ", round(summary(model_step)$adj.r.squared, 4))
message("Final Model Adj R²: ", round(summary(final_model)$adj.r.squared, 4))

# Compare
if (summary(model_step)$adj.r.squared > summary(final_model)$adj.r.squared + 0.01) {
  message("\nNote: Stepwise model shows improvement - consider reviewing")
} else {
  message("\nFinal model selection confirmed - similar or better performance")
}
