...
The dataset contains **`r nrow(data_clean)` observations** and **`r ncol(data_clean) - 1` variables** (excluding ID), including:
...
```{r}
#| label: tbl-response-metrics
#| tbl-cap: "Shape metrics for exam scores"

shapiro_result <- shapiro.test(train_data$y)
skew <- skewness(train_data$y)
kurt <- kurtosis(train_data$y)

tibble(
  Metric = c("Shapiro-Wilk p-value", "Skewness", "Kurtosis"),
  Value = c(round(shapiro_result$p.value, 4), 
            round(skew$Skewness, 4), 
            round(kurt$Kurtosis, 4))
  ) |>
  kable() |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover"))
```
...
-   Shapiro-Wilk test (p = `r round(shapiro_result$p.value, 4)`) suggests some deviation from normality, while the symmetric shape and absence of skewness indicate that it is not problematic for the Gaussian assumption of response variable in linear regression
...
-   Sleep quality shows ordinal relationship with exam scores
-   Private school students outperform public school students on average
-   Higher parental education levels associate with better student performance
-   Participation in extracurricular activities correlates with higher exam scores
-   We should consider ordinal coding for ordered categorical predictors in modeling
...
#| fig-cap: "Visual check for key interactions"
...
p_int_study_sleep | p_int_attend_school
```
...
model_minimal <- lm(y ~ study_hrs + attend_pct + sleep_qual + parent_educ, data = train_data)
...
# Model with interactions (based on EDA hypotheses)
...
# Refined model (removed non-significant interaction)
...
# Function to calculate test set RMSE
calc_test_rmse <- function(model) {
  preds <- predict(model, newdata = test_data)
  sqrt(mean((test_data$y - preds)^2))
}

# Get model metrics including test set performance
comparison_table <- map_dfr(names(models), function(name) {
  ...
  tibble(
    ...
    AIC = AIC(m),
    BIC = BIC(m),
    `Test RMSE` = calc_test_rmse(m)
  )
})

# Select final model based on minimum test RMSE (best prediction power)
best_model_name <- comparison_table$Model[which.min(comparison_table$`Test RMSE`)]
final_model <- models[[best_model_name]]
```

## Model Comparison

We compared six candidate models:

1.  **Null model**: Intercept only (baseline)
2.  **Minimal**: Top 4 predictors from EDA (`study_hrs`, `attend_pct`, `sleep_qual`, `parent_educ`)
3.  **Main effects**: Minimal predictors + `sleep_hrs` + `school_type` + `extra_act` without interactions
4.  **Full model**: All available predictors
5.  **Interaction**: Main effects + `study_hrs:sleep_qual` + `attend_pct:school_type`
6.  **New**: Main effects + `study_hrs:sleep_qual` (refined after removing non-significant interaction)

```{r}
#| label: tbl-model-comparison
#| tbl-cap: "Comparison of candidate regression models (selected by minimum Test RMSE)"

comparison_table |>
  mutate(across(where(is.numeric), ~round(.x, 4))) |>
  kable() |>
  kable_styling(full_width = FALSE, bootstrap_options = c("striped", "hover")) |>
  row_spec(which.min(comparison_table$`Test RMSE`), bold = TRUE, background = "#e6f3ff")
```

## Final Model Selection

Based on test set predictive performance (minimum RMSE), we selected the **`r best_model_name`** model:

**Model Formula:** `r deparse(formula(final_model)) |> paste(collapse = " ")`
...
We define 2 profiles of students: **high-achieving profile** (high study time, strong attendance, good sleep, private schooling, high parental education, and extracurricular participation), and **at-risk profile** with less favorable characteristics. Predicted scores differ substantially between high-achieving and at-risk students, with non-overlapping 95% confidence intervals, highlighting the cumulative effects of academic engagement, sleep, and family background.
...
The following AI tools were used in the development of this report:
...
-   **Visualization and Table making**: AI wrote codes concerning visualizations and tables.
